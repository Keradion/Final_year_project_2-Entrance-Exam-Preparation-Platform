/**
 * Seed Grade 10 Mathematics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade10-math.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade10-math.js --reset   # remove existing Grade 10 Math Natural + re-seed
 *
...
const curriculum = require('./data/grade10MathematicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade10SeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade10ExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade10MathNatural() {
  ...
  console.log('Removed existing Grade 10 Mathematics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade10MathNatural();
  else {
  ...
  console.log('\n✅ Grade 10 Mathematics seed complete');
