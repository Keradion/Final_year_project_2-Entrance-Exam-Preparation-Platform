/**
 * Seed Grade 12 Mathematics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade12-math.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade12-math.js --reset   # remove existing Grade 12 Math Natural + re-seed
 *
...
const curriculum = require('./data/grade12MathematicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade12SeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade12ExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade12MathNatural() {
  ...
  console.log('Removed existing Grade 12 Mathematics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade12MathNatural();
  else {
  ...
  console.log('\n✅ Grade 12 Mathematics seed complete');
