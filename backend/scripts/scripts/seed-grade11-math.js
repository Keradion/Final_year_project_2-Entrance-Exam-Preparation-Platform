/**
 * Seed Grade 11 Mathematics (Natural stream, MoE-aligned curriculum data).
 *
 * Usage:
 *   node scripts/scripts/seed-grade11-math.js           # insert (fails if subject already exists)
 *   node scripts/scripts/seed-grade11-math.js --reset   # remove existing Grade 11 Math Natural + re-seed
 *
...
const curriculum = require('./data/grade11MathematicsCurriculum');
const { exercisesForTopic, buildTopicStudyNotes, pickRandomTopicVideo } = require('./data/grade11SeedContent');
const { buildExamQuestionsForTopic } = require('./data/grade11ExamQuestions');
const {
  User,
  Subject,
  Chapter,
...
async function purgeGrade11MathNatural() {
  ...
  console.log('Removed existing Grade 11 Mathematics (Natural) seed data.');
}
...
async function main() {
  ...
  if (RESET) await purgeGrade11MathNatural();
  else {
  ...
  console.log('\n✅ Grade 11 Mathematics seed complete');
