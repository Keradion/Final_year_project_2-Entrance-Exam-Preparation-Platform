...
const TopicObjectives = () => {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 py-6">
      <div className={`${isStudent ? 'bg-white p-0 border-0 shadow-none' : 'bg-white p-10 rounded-xl border border-outline-variant shadow-[0px_8px_24px_rgba(0,0,0,0.08)]'}`}>
        {!isStudent && (
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-2xl font-bold text-on-surface">Topic Objectives</h3>
              <p className="text-on-surface-variant/60 text-sm font-medium mt-1">Define what students should achieve after completing this topic.</p>
            </div>
        ...
        <div className={isStudent ? '' : 'space-y-4'}>
          {topicObjectives.length > 0 ? (
            isStudent ? (
              <ul className="list-disc pl-6 space-y-3 text-base leading-8 text-on-surface">
                {topicObjectives.map((obj, i) => (
                  <li key={i}>{obj}</li>
                ))}
              </ul>
            ) : topicObjectives.map((obj, i) => (
              <div key={i} className="flex items-center justify-between p-6 bg-surface rounded-xl border border-outline/5 group hover:border-primary-container/30 hover:bg-white hover:shadow-md transition-all duration-300">
                <div className="flex items-center gap-6 flex-grow">
                  <div className="w-10 h-10 rounded-lg bg-primary-container/10 flex items-center justify-center text-primary-container font-black text-xs border border-primary-container/10 shrink-0">
                    {i + 1}
                  </div>
                  {editingIndex === i ? (
                    <div className="flex items-center gap-3 flex-grow">
                      <input 
                        ...
                        className="flex-grow bg-white border border-primary-container/40 px-4 py-2 rounded-lg font-bold text-on-surface outline-none" 
                        autoFocus
                      />
                      <button onClick={handleSaveEdit} className="p-2 bg-primary-container text-white rounded-lg hover:brightness-110"><Check size={18} /></button>
                      <button onClick={handleCancelInlineEdit} className="p-2 bg-surface border border-outline/10 text-outline rounded-lg hover:bg-outline/5"><X size={18} /></button>
                    </div>
                  ) : (
                    <span className="font-bold text-on-surface text-lg">{obj}</span>
                  )}
                </div>
            ...
            isStudent ? (
              <p className="text-on-surface-variant">No objectives defined yet.</p>
            ) : (
              <div className="text-center py-20 bg-surface/50 rounded-xl border border-dashed border-outline/20 opacity-40">
                ...
                <p className="font-bold text-lg uppercase tracking-widest">No objectives defined yet.</p>
              </div>
            )
          )}
        </div>
